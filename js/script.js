let CACHES = {
    ...
    "cn": {
        texts: {
            "page-title": "米雪儿洗大锅",
            "doc-title": "洗大锅~",
            "page-descriptions": "给米雪儿写的小网站，对，就是《卡拉彼丘》欧泊的 <del>糖猫</del> 搜查官！",
            "counter-descriptions": ["米雪儿已经洗了", "米雪儿已经似了"],
            "counter-unit": ["次"],
            "counter-button": ["加载中...", "请稍候..."],
            "repository-desc": "GitHub 仓库"
        },
        audioList: [
            "audio/DM_20260630184514_001.mp3",
            "audio/DM_20260630184541_001.mp3",
            "audio/DM_20260630184626_001.mp3",
            "audio/DM_20260630184724_001.mp3"
        ],
        cardImage: "img/c3dd5aac727c27ef87c364e07344e7910463a618.jpg"
    },
    gifs: [
        "img/bg.png",
        "img/pj1.png",
...
(() => {
    ...
    async function cacheFilesToBase64(dict) {
        ...
        if (dict.hasOwnProperty(lang)) {
            ...
            dict[lang].texts["counter-button"] = ["洗大锅~", "哎嘿~"];
        }
    ...
    window.onload = function () {
        ...
        cacheFilesToBase64(CACHES).catch(error => {
            ...
            counterButton.innerText = "哎嘿~";
        });
        $("#show-options-opt").on("click", function () {
            window.open("https://github.com/duiqt/herta_kuru", "_blank");
        });
        $("#show-gif-src").on("click", function () {
            window.open("https://www.bilibili.com/opus/756101504876675091?spm_id_from=333.1387.0.0", "_blank");
        });
        $("#show-audio-src").on("click", function () {
            window.open("https://wiki.biligame.com/klbq/%E7%B1%B3%E9%9B%AA%E5%84%BF%C2%B7%E6%9D%8E/%E8%AF%AD%E9%9F%B3%E5%8F%B0%E8%AF%8D#%E5%AF%B9%E5%B1%80", "_blank");
        });
    }
